<?php
class Model_incoming extends CI_Model{
...
//  kode job
    ...
    {
		$bulan=date('m');
        $query = $this->db->query("select MAX(RIGHT(HouseNo,4)) as kd_max from incoming_house WHERE MID(HouseNo,7,2)='$bulan'");
        $kd = "";
        if($query->num_rows()>0)
        {
            foreach($query->result() as $k)
        ...
        return "IN".date('Ym').$kd;
}
//    kode house
    public function getJob($status)
    {
		$bulan=date('m');
        $query = $this->db->query("select MAX(RIGHT(JobNo,5)) as kd_max from incoming_house WHERE MID(JobNo,8,2)='$bulan'");
        $kd = "";
        if($query->num_rows()>0)
        {
            foreach($query->result() as $k)
